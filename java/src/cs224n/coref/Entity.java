...
public class Entity implements Serializable, Decodable, Iterable<Mention> {
  ...
  **/
  ...
  // Formats a mention given that it refers to this entity, and has been guessed to belong
  // to the Entity supplied as an argument
  protected StringBuilder format(StringBuilder b, Mention mention, Entity guessEntity ){
    if(Ansi.supportsAnsi()){
      b.append(Ansi.prefix(guessEntity.uniqueID)).append("{{").append(Ansi.endFormatting);
      b.append(Ansi.prefix(uniqueID)).append(mention.toString()).append(Ansi.endFormatting);
      b.append(Ansi.prefix(guessEntity.uniqueID)).append("}}").append(Ansi.endFormatting);
      return b;
    } else {
      return b.append("[").append(uniqueID).append("]{").append(mention.toString()).append("}");
