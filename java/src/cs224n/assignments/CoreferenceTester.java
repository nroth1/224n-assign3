...
//      if(Math.abs(recall-check) > 1e-6){ throw new IllegalStateException("MUC scores do not agree (not your fault)!"); }
  ...
  public String debug(File[] data, Properties props){
    ...
    for(File f : data){
      ...
      //(enter score)
      Collection<Entity> gold = datum.goldClusters;
      debug.append("=====Document " + doc.id.replaceAll("/",".") + "=====\n").append("These are your versions of the gold clusters, with same-colored mentions indicating the proper gold clusters for improperly clustered words and black mentions currently missing from the cluster:").append("\n").append(doc.debug(guess,gold)).append("\n").append("These are the gold clusters, with same-colored mentions indicating your clusters:").append("\n");

      //Now we try to print out guess and gold in reverse - we need to obtain the list of entities corresponding to the guess clusters and the list of clustered mentions corresponding to the gold clusters
	...
	debug.append("**Caution: your guess contains more than 20 unique entities, so some colors/styles may be repeated due to ANSI limitations.\n");
      }
      debug.append("\n\n");
    }
    //--Return
  ...
  public static void main(String[] args) {
    ...
    //System.out.println(systemClass + "HHHHHHHHHHHHH!!!!!!!!!!!!!!!!!!!!");
    //(create)
