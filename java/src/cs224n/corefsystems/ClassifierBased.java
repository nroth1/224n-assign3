...
//    Pair.make(Feature.FuzzyMatch.class, Feature.EarlyMatch.class) <- improves performance, but questionable interpretation
  ...
  private boolean isFuzzy(Mention onPrix, Mention candidate) {
    ...
    List<Token> tokens2 = candidate.sentence.tokens;
    List<Token> tokensCandidate = new ArrayList<Token>();

    for (int i = candidate.beginIndexInclusive; i < candidate.endIndexExclusive; i++) {
  ...
  private double fuzzyFrac(Mention onPrix, Mention candidate) {
    List<Token> tokens = onPrix.sentence.tokens;
    List<Token> tokens2 = candidate.sentence.tokens;
    List<Token> tokensCandidate = new ArrayList<Token>();
    ...
    double frac = ((double) overlap) / ((double) union);
...
//    System.out.println(isEarly && isEarlyCand);
  ...
  private boolean isPossible(Mention onPrix, Mention candidate) {
    ...
    if (!getSpeaker(onPrix).equals(getSpeaker(candidate))) {
      return false;
    }

    if (getGender(onPrix) != getGender(candidate)) {
      return false;
    }
    return true;
  }

  private boolean isHeadMatch(Mention onPrix, Mention candidate) {
    return onPrix.headWord().equalsIgnoreCase(candidate.headWord());
  }
  ...
  public FeatureExtractor<Pair<Mention, ClusteredMention>, Feature, Boolean> extractor = new FeatureExtractor<Pair<Mention, ClusteredMention>, Feature, Boolean>() {
    private <E> Feature feature(Class<E> clazz, Pair<Mention, ClusteredMention> input, Option<Double> count) {
      ...
      if (clazz.equals(Feature.ExactMatch.class)) {
        ...
        return new Feature.HeadMatch(isHeadMatch(onPrix, candidate));
      } else {
